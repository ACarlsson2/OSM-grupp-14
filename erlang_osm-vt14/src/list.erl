...
pmax(List, N, Death) when length(List) > N ->
    Lists = split(List, N),
    CollectPID = self(),    
    WorkerList = [{spawn_link(fun() -> worker(L, CollectPID, Death) end), L} || L <- Lists],
    io:format("Workers = ~w~n", [WorkerList]),
    Maxes = collect(length(Lists), [], Death, WorkerList),
    pmax(Maxes, N, Death);
...
collect(N, Maxes, Death, WorkerList) when length(Maxes) < N ->
    receive 
    {'EXIT', PID, random_death} ->
        % Restart process with PID here
        CollectPID = self(),
        {_,{_, L},NewWorkerList} = lists:keytake(PID, 1, WorkerList),
        collect(N, Maxes, Death, [{spawn_link(fun() -> worker(L, CollectPID, Death) end), L}|NewWorkerList]);
    {'EXIT', _PID, normal} ->
        collect(N, Maxes, Death, WorkerList);
    Max -> 
        collect(N, [Max|Maxes], Death, WorkerList) 
    end;
