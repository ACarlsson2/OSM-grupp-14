...
simpsons(names) ->
  ...
  lists:sort(L);
simpsons(males) ->
    L = [N || {_,G,N} <- simpsons(), G == male],
    lists:sort(L);
simpsons(females) ->
    L = [N || {_,G,N} <- simpsons(), G == female],
    lists:sort(L);
simpsons(pets) ->
    L = [N || {T,_,N} <- simpsons(), T == dog orelse T == pig orelse T == cat],
    lists:sort(L).
...
max([H | T]) ->
    F = fun(A, R) -> if A > R -> A; true -> R end end,
    lists:foldl(F, H, T).
...
count(String, Char) ->
    ...
    F = fun(J, A) ->  if Char == J -> A+1; true -> A end end,
